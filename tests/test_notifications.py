...
def test_filing_failed_message_truncates_long_reasons():
    ...
    assert body == f"scan001.pdf → _Review_Needed\nReason: {'x' * MAX_REASON_CHARS}…"
...
def test_scan_stranded_message_exact_wording():
    ...
    assert title == "ScanSort needs attention"
    assert body == (
        "scan001.pdf could not be processed or moved to _Review_Needed.\n"
...
def test_notify_file_filed_dispatches_built_message():
    with patch("scansort.notifications.show_toast", return_value=True) as mock_toast:
        assert notify_file_filed("doc.pdf", "Bills") is True
    mock_toast.assert_called_once_with("Document filed", "doc.pdf → Bills")
...
def test_notify_filing_failed_dispatches_built_message():
    with patch("scansort.notifications.show_toast", return_value=False) as mock_toast:
        assert notify_filing_failed("a.pdf", "_Review_Needed", "boom") is False
    mock_toast.assert_called_once_with(
        "Document filing failed", "a.pdf → _Review_Needed\nReason: boom"
    )


def test_notify_scan_stranded_dispatches_built_message():
    with patch("scansort.notifications.show_toast", return_value=True) as mock_toast:
        assert notify_scan_stranded("a.pdf", "_Review_Needed") is True
    mock_toast.assert_called_once_with(
        "ScanSort needs attention",
        "a.pdf could not be processed or moved to _Review_Needed.\n"
        "Please check your drop folder.",
    )
