...
from scansort.core.constants import DEFAULT_DESCRIPTION, MAX_DESCRIPTION_LENGTH
...
class _ClassificationFields(BaseModel):
    """Fields shared by the Gemini API schema and the ScanSort domain model."""
    ...
    folder_reasoning: str = Field(
        default="",
        description="Crisp 1-sentence explanation of why the target folder was chosen",
    )
...
class GeminiClassificationResponse(_ClassificationFields):
    """Structured classification output generated by the Gemini API.

    Passed verbatim as ``response_schema``; it deliberately exposes only the
    model-visible fields and never the pipeline telemetry carried by
...
class DocumentClassification(_ClassificationFields):
    """Domain classification model used by ScanSort pipeline and audit logger."""
    ...
    prompt_tokens: int = Field(
        default=0, ge=0, description="Prompt token count from Gemini API"
    )
    candidates_tokens: int = Field(
        default=0, ge=0, description="Candidates token count from Gemini API"
    )
    estimated_cost_usd: str | float | None = Field(
        default=None, description="Formatted estimated USD cost string or float"
    )

    @property
    def total_tokens(self) -> int:
        """Sum of prompt and candidate tokens consumed."""
